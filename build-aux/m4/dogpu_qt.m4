...
dnl Copyright (c) 2021 The Dogegpu Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set dogpu_enable_qt to false. Else, exit.
AC_DEFUN([DOGPU_QT_FAIL],[
  if test "x$dogpu_qt_want_version" = xauto && test "x$dogpu_qt_force" != xyes; then
    if test "x$dogpu_enable_qt" != xno; then
      AC_MSG_WARN([$1; dogpu-qt frontend will not be built])
    fi
    dogpu_enable_qt=no
    dogpu_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])

AC_DEFUN([DOGPU_QT_CHECK],[
  if test "x$dogpu_enable_qt" != xno && test "x$dogpu_qt_want_version" != xno; then
    true
    $1
  else
    true
...
dnl DOGPU_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([DOGPU_QT_PATH_PROGS],[
  DOGPU_QT_CHECK([
    ...
    if test "x$$1" = x && test "x$4" != xyes; then
      DOGPU_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other DOGPU_QT* macros to ensure that
dnl input variables are set correctly.
dnl CAUTION: Do not use this inside of a conditional.
AC_DEFUN([DOGPU_QT_INIT],[
  dnl enable qt support
  AC_ARG_WITH([gui],
    [AS_HELP_STRING([--with-gui@<:@=no|qt5|auto@:>@],
    [build dogpu-qt GUI (default=auto)])],
    [
     dogpu_qt_want_version=$withval
     if test "x$dogpu_qt_want_version" = xyes; then
       dogpu_qt_force=yes
       dogpu_qt_want_version=auto
     fi
    ],
    [dogpu_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); dogpu_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   DOGPU_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _DOGPU_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: dogpu_enable_qt, dogpu_enable_qt_dbus, dogpu_enable_qt_test
AC_DEFUN([DOGPU_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  DOGPU_QT_CHECK([_DOGPU_QT_FIND_LIBS])

  dnl This is ugly and complicated. Yuck. Works as follows:
  dnl For Qt5, we can check a header to find out whether Qt is build
  dnl statically. When Qt is built statically, some plugins must be linked into
  dnl the final binary as well.
  dnl _DOGPU_QT_CHECK_STATIC_PLUGIN does a quick link-check and appends the
  ...
  _DOGPU_QT_IS_STATIC
  if test "x$dogpu_cv_static_qt" = xyes; then
    _DOGPU_QT_CHECK_STATIC_LIBS
    ...
    if test "x$TARGET_OS" != xandroid; then
      _DOGPU_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _DOGPU_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      ...
      _DOGPU_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _DOGPU_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _DOGPU_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  DOGPU_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  DOGPU_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  DOGPU_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  DOGPU_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  DOGPU_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  ...
  DOGPU_QT_CHECK([
    dogpu_enable_qt=yes
    dogpu_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      dogpu_enable_qt_test=no
    fi
    dogpu_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      dogpu_enable_qt_dbus=yes
    fi
  ...
  if test x$dogpu_enable_qt = xyes; then
    AC_MSG_RESULT([$dogpu_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$dogpu_enable_qt])
  fi
...
dnl Output: dogpu_cv_static_qt=yes|no
AC_DEFUN([_DOGPU_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, dogpu_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [dogpu_cv_static_qt=yes],
      [dogpu_cv_static_qt=no])
    ])
])

dnl Internal. Check if the link-requirements for a static plugin are met.
dnl
dnl _DOGPU_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_DOGPU_QT_CHECK_STATIC_PLUGIN], [
...
AC_DEFUN([_DOGPU_QT_FIND_LIBS],[
  DOGPU_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [],
                      [DOGPU_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  DOGPU_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [],
                      [DOGPU_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  DOGPU_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [],
                      [DOGPU_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  DOGPU_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [],
                      [DOGPU_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
