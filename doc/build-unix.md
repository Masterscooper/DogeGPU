...
====================
Some notes on how to build Dogpu Core in *nix.


Note
---------------------
Always use absolute paths to configure and compile dogpu and the dependencies,
...
This will build dogpu-qt as well if the dependencies are met.
...
Notes
-----
The release is built with GCC and then "strip dogpud" to strip the debug
...
Security
--------
To help make your dogpu installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, dogpu should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, dogpu may be compiled in
...
`--with-incompatible-bdb` according to the [PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/dogpu/trunk/PKGBUILD).
As mentioned above, when maintaining portability of the wallet between the standard Dogpu Core distributions and independently built
