...
The Dogpu Core daemon can be configured to act as a trusted "border
router", implementing the dogpu wire protocol and relay, making
...
The ZeroMQ feature in Dogpu Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building dogpud:
...
For instance:
    ...
    $ dogpud -zmqpubhashtx=tcp://127.0.0.1:26068 \
               -zmqpubrawtx=ipc:///tmp/dogpud.tx.raw
...
These options can also be provided in dogpu.conf.
...
From the perspective of dogpud, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into dogpud directly. Furthermore, no information is
...
using. Dogpud appends an up-counting sequence number to each
