...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Dogpu address or script");
    }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The dogpu address for the private key\n"
            "\nResult:\n"
    ...
    return CDogpuSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to dogpud)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    // produce output
    file << strprintf("# Wallet dump created by Dogpu %s\n", CLIENT_BUILD);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (!pwallet->GetHDChain().IsBip44()) {
            ...
            if (pwallet->GetKey(seed_id, seed)) {
                ...
                CDogpuExtKey b58extkey;
                b58extkey.SetKey(masterKey);

                CExtPubKey pubkey;
                pubkey = masterKey.Neuter();

                CDogpuExtPubKey b58extpubkey;
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CDogpuSecret(key).ToString(), strTime);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (!pwallet->GetHDChain().IsBip44()) {
            ...
            if (pwallet->GetKey(seed_id, seed)) {
                ...
                // Get the Dogpu Ext Key from the master key
                CDogpuExtKey b58extkey;
                b58extkey.SetKey(masterKey);
                ...
                // Get the Dogpu Ext Key from the public key
                CDogpuExtPubKey b58extpubkey;
                b58extpubkey.SetKey(pubkey);
                ...
                // Create the Dogpu Account Ext Private Key
                CDogpuExtKey b58accountextprivatekey;
                b58accountextprivatekey.SetKey(accountKey);

                // Create the Dogpu Account Ext Public Key
                CDogpuExtPubKey b58actextpubkey;
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see dogpud log for details) and could "
