...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for DOGPUS
    ...
    if (p_rewardSnapshot.strDistributionAsset != "DOGPUS") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is DOGPUS with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using DOGPUS differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "DOGPUS") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Dogpu address (already validated during ownership snapshot creation)
