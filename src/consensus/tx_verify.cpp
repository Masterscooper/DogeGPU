...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs, bool fMempoolCheck, bool fBlockCheck)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        /** DOGPUS START */
        // Find and handle all new OP_DOGPUS_ASSET null data transactions
    ...
    if (tx.IsNewAsset()) {
        ...
        // above transaction types.  Also fail if it contains OP_DOGPUS_ASSET opcode but wasn't a valid script.
        for (auto out : tx.vout) {
            int nType;
            bool _isOwner;
            if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                ...
                if (out.scriptPubKey.Find(OP_DOGPUS_ASSET)) {
                    if (out.scriptPubKey[0] != OP_DOGPUS_ASSET) {
                        return state.DoS(100, false, REJECT_INVALID,
                                         "bad-txns-op-dogpus-asset-not-in-right-script-location");
                    }
                }
            }
        }
                ...
                if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                    ...
                    if (out.scriptPubKey.Find(OP_DOGPUS_ASSET)) {
                        if (AreRestrictedAssetsDeployed()) {
                            if (out.scriptPubKey[0] != OP_DOGPUS_ASSET) {
                                return state.DoS(100, false, REJECT_INVALID,
                                                 "bad-txns-op-dogpus-asset-not-in-right-script-location", false, "", tx.GetHash());
                            }
                        } else {
