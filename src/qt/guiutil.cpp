...
#include "dogpuaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Dogpu address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new DogpuAddressEntryValidator(parent));
    widget->setCheckValidator(new DogpuAddressCheckValidator(parent));
}
...
bool parseDogpuURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no dogpu: URI
    if(!uri.isValid() || uri.scheme() != QString("dogpu"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!DogpuUnits::parse(DogpuUnits::DOGPUS, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseDogpuURI(QString uri, SendCoinsRecipient *out)
{
    // Convert dogpu:// to dogpu:
    //
    //    Cannot handle this later, because dogpu:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("dogpu://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "dogpu:");
    }
    QUrl uriInstance(uri);
    return parseDogpuURI(uriInstance, out);
}

QString formatDogpuURI(const SendCoinsRecipient &info)
{
    QString ret = QString("dogpu:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(DogpuUnits::format(DogpuUnits::DOGPUS, info.amount, false, DogpuUnits::separatorNever));
        paramCount++;
    }
...
bool openDogpuConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(DOGPU_CONF_FILENAME);
    ...
    /* Open dogpu.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Dogpu.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Dogpu (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Dogpu (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "dogpu.desktop";
    return GetAutostartDir() / strprintf("dogpu-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a dogpu.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Dogpu\n";
        else
            optionFile << strprintf("Name=Dogpu (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the dogpu app
...
bool GetStartOnSystemStartup()
{
    CFURLRef dogpuAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (dogpuAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, dogpuAppUrl);

    CFRelease(dogpuAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add dogpu app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, dogpuAppUrl, nullptr, nullptr);
    }
