        ...
        if(fIsSingleSign)
        {
            ...
            else //We were the ones executing the transaction
            {
                ...
                for(const CTxOut &txout : wtx.tx->vout)
                {
                    if(wallet->IsMine(txout))
                    {
                        //If we sent assets, we need to see if we were sent assets or DOGPUS in return
                        if(txout.scriptPubKey.IsAssetScript())
                        {
            ...
            if(fSentAssets && fRecvAssets) {
                ...
                std::string asset_qty_format = DogpuUnits::formatWithCustomName(QString::fromStdString(sentType), sentAmount, 2).toUtf8().constData();
                ...
                std::string asset_qty_format = DogpuUnits::formatWithCustomName(QString::fromStdString(sentType), sentAmount, 2).toUtf8().constData();
                sub.assetName = strprintf("DOGPUS (%s %s)", TransactionView::tr("Sold").toUtf8().constData(), asset_qty_format);
            } else if (fRecvAssets) {
                //Buy!
                //Total price paid, need to use net calculation when we executed
                sub.credit = (mine ? -myProvidedInput.nValue : nNet);
                std::string asset_qty_format = DogpuUnits::formatWithCustomName(QString::fromStdString(recvType), recvAmount, 2).toUtf8().constData();
                sub.assetName = strprintf("DOGPUS (%s %s)", TransactionView::tr("Bought").toUtf8().constData(), asset_qty_format);
            } else {
