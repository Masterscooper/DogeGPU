...
const std::string DogpuGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString DogpuGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "DOGPUSBTC"  , 1,          8},
    { "mBTC",   "DOGPUSBTC"  , 1000,       5},
    { "µBTC",   "DOGPUSBTC"  , 1000000,    2},
    { "Satoshi","DOGPUSBTC"  , 100000000,  0},
    { "USDT",   "DOGPUSUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(DogpuGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

DogpuGUI::DogpuGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void DogpuGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Dogpu address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and dogpu: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to DOGPUS addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Dogpu addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Dogpu addresses"));
    ...
    openAction->setStatusTip(tr("Open a dogpu: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Dogpu command-line options").arg(tr(PACKAGE_NAME)));
...
void DogpuGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/dogpustext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/dogegputext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* dogpuLabelLayout = new QVBoxLayout(toolbarWidget);
        dogpuLabelLayout->addWidget(labelToolbar);
        dogpuLabelLayout->addWidget(m_toolbar);
        dogpuLabelLayout->setDirection(QBoxLayout::TopToBottom);
        dogpuLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("DogeGPU Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/Masterscooper/Dogegpu/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Dogegpu release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/Masterscooper/Dogegpu/releases";
...
void DogpuGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/dogpustext")));
        m_toolbar->setMaximumWidth(65);
...
void DogpuGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool DogpuGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool DogpuGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void DogpuGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void DogpuGUI::setWalletActionsEnabled(bool enabled)
{
...
void DogpuGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void DogpuGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void DogpuGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void DogpuGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void DogpuGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void DogpuGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void DogpuGUI::openClicked()
{
...
void DogpuGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void DogpuGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void DogpuGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void DogpuGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void DogpuGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void DogpuGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** DOGPUS START */
void DogpuGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void DogpuGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void DogpuGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void DogpuGUI::gotoRestrictedAssetsPage()
{
...
void DogpuGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Dogpu network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void DogpuGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void DogpuGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void DogpuGUI::updateHeadersSyncProgressLabel()
{
...
void DogpuGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void DogpuGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Dogpu"); // default title
...
void DogpuGUI::changeEvent(QEvent *e)
{
...
void DogpuGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void DogpuGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void DogpuGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "DOGPUS")
        msg += tr("Amount: %1\n").arg(DogpuUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(DogpuUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void DogpuGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void DogpuGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool DogpuGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool DogpuGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void DogpuGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void DogpuGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void DogpuGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void DogpuGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void DogpuGUI::detectShutdown()
{
...
void DogpuGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void DogpuGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void DogpuGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(DogpuGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void DogpuGUI::subscribeToCoreSignals()
{
...
void DogpuGUI::unsubscribeFromCoreSignals()
{
...
void DogpuGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void DogpuGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<DogpuUnits::Unit> units = DogpuUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const DogpuUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(DogpuUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(DogpuUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (DogpuUnits::Unit u : DogpuUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(DogpuUnits::name(u)), this);
...
{
    setText(DogpuUnits::name(newUnits));
}
...
void DogpuGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void DogpuGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "DogpuGUI::onPriceUnitChange: " + QString::number(newIndex);
...
void DogpuGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/Masterscooper/dogegpu/releases"));
