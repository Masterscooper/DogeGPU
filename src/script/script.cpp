...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** DOGPUS START */
    case OP_DOGPUS_ASSET              : return "OP_DOGPUS_ASSET";
    /** DOGPUS END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_DOGPUS_ASSET) { // OP_DOGPUS_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == DOGPUS_R) { // Check to see if DOGPUS starts at 27 ( this->size() < 105)
                if ((*this)[28] == DOGPUS_V)
                    if ((*this)[29] == DOGPUS_N)
                        index = 30;
            } else {
                if ((*this)[28] == DOGPUS_R) // Check to see if DOGPUS starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == DOGPUS_V)
                        if ((*this)[30] == DOGPUS_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == DOGPUS_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == DOGPUS_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == DOGPUS_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == DOGPUS_R) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_DOGPUS_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_DOGPUS_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_DOGPUS_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_DOGPUS_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_DOGPUS_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_DOGPUS_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
