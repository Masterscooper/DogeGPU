...
{
    return AreAssetsDeployed() ? "" : "\nTHIS COMMAND IS NOT YET ACTIVE!\nhttps://github.com/DogpuProject/rips/blob/master/rip-0002.mediawiki\n";
}
...
UniValue UpdateAddressTag(const JSONRPCRequest &request, const int8_t &flag)
{
    ...
    if (!IsValidDestination(destination)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Dogpu address: ") + address);
    }
    ...
    if (request.params.size() > 2) {
        ...
        if (!change_address.empty()) {
           ...
           if (!IsValidDestination(change_dest)) {
               throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Dogpu change address: ") + change_address);
           }
        }
    }
...
UniValue issue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "4. \"change_address\"        (string), optional, default=\"\"), address the the dogpus change will be sent to, if it is empty, change address will be generated for you\n"
        ...
        if (!change_address.empty()) {
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                                   std::string("Invalid Change Address: Invalid Dogpu address: ") + change_address);
            }
        }
    }
...
UniValue issueunique(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "Five (5) DOGPUS will be burned for each asset created.\n"
                ...
                "5. \"change_address\"        (string, optional, default=\"\"), address the the dogpus change will be sent to, if it is empty, change address will be generated for you\n"
    ...
    if (!changeAddress.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Change Address: Invalid Dogpu address: ") + changeAddress);
        }
    }
...
UniValue listassetbalancesbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                  (string, required) a dogpu address\n"
...
UniValue transfer(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
                ...
                "6. \"change_address\"       (string, optional, default = \"\") the transactions DOGPUS change will be sent to this address\n"
    ...
    if (!IsValidDestination(to_dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Dogpu address: ") + to_address);
    }
    ...
    std::string dogpus_change_address = "";
    if (request.params.size() > 5) {
        dogpus_change_address = request.params[5].get_str();
    }
    ...
    CTxDestination dogpus_change_dest = DecodeDestination(dogpus_change_address);
    if (!dogpus_change_address.empty() && !IsValidDestination(dogpus_change_dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("DOGPUS change address must be a valid address. Invalid address: ") + dogpus_change_address);
    ...
    CCoinControl ctrl;
    ctrl.destChange = dogpus_change_dest;
...
UniValue transferfromaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
            "transferfromaddresses \"asset_name\" [\"from_addresses\"] qty \"to_address\" \"message\" expire_time \"dogpus_change_address\" \"asset_change_address\"\n"
            + AssetActivationWarning() +
            ...
            "7. \"dogpus_change_address\"       (string, optional, default = \"\") the transactions DOGPUS change will be sent to this address\n"
    ...
    std::string dogpus_change_address = "";
    if (request.params.size() > 6) {
        dogpus_change_address = request.params[6].get_str();
    }
...
UniValue transferfromaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
                "transferfromaddress \"asset_name\" \"from_address\" qty \"to_address\" \"message\" expire_time \"dogpus_change_address\" \"asset_change_address\"\n"
                + AssetActivationWarning() +
                ...
                "5. \"message\"                  (string, optional) Once RIP5 is voted in ipfs hash or txid hash to send along with the transfer\n"
                "6. \"expire_time\"              (numeric, optional) UTC timestamp of when the message expires\n"
                "7. \"dogpus_change_address\"       (string, optional, default = \"\") the transaction DOGPUS change will be sent to this address\n"
...
UniValue listtagsforaddress(const JSONRPCRequest &request)
{
    ...
    CTxDestination dest = DecodeDestination(address);
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Not valid DOGPUS address: ") + address);
...
UniValue checkaddresstag(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() != 2)
        throw std::runtime_error(
                ...
                "\nArguments:\n"
                "1. \"address\"          (string, required) the DOGPUS address to search\n"
...
UniValue issuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 4 || request.params.size() > 9)
        throw std::runtime_error(
                ...
                "5. \"change_address\"        (string, optional, default=\"\") address that the dogpus change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue reissuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
                ...
                "6. \"change_address\"        (string, optional, default=\"\") address that the dogpus change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue transferqualifier(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() > 3) {
        ...
        CTxDestination change_dest = DecodeDestination(change_address);
        if (!IsValidDestination(change_dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Dogpu address: ") + change_address);
        }
    }
...
#ifdef ENABLE_WALLET
    { "assets",   "transferfromaddress",        &transferfromaddress,        {"asset_name", "from_address", "qty", "to_address", "message", "expire_time", "dogpus_change_address", "asset_change_address"}},
    { "assets",   "transferfromaddresses",      &transferfromaddresses,      {"asset_name", "from_addresses", "qty", "to_address", "message", "expire_time", "dogpus_change_address", "asset_change_address"}},
