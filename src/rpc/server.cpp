...
CAmount AmountFromValue(const UniValue& value, bool p_isDOGPUS)
{
    ...
    if (p_isDOGPUS && !MoneyRange(amount))
...
{
    return "> dogpu-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:6068/\n";
}
