...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total Dogegpu balance of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given dogpu address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The dogpu address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The dogpu address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Dogpu address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are dogpu addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) dogpu address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The dogpu address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "dogpu-cli and the GUI. There is no server-side difference."
        );
...
{
    if (type == 2) {
        address = CDogpuAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CDogpuAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CDogpuAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CDogpuAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  {\n"
            "    \"address\"  (string) The base58check encoded address\n"
            "    \"assetName\"  (string) The name of the associated asset (DOGPUS for Dogegpu)\n"
    ...
    if (includeAssets) {
        ...
        if (!mempool.getAddressIndex(addresses, DOGPUS, indexes)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
        }
    }
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"assetName\"   (string, optional) Get UTXOs for a particular asset instead of DOGPUS ('*' for all assets).\n"
            "}\n"
            "\nResult\n"
            "[\n"
            "  {\n"
            "    \"address\"  (string) The address base58check encoded\n"
            "    \"assetName\" (string) The asset associated with the UTXOs (DOGPUS for Dogegpu)\n"
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++) {
        ...
        std::string assetNameOut = "DOGPUS";
        if (assetName != "DOGPUS") {
            CAmount _amount;
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1 || !request.params[0].isObject())
        throw std::runtime_error(
            ...
            "  \"assetName\"   (string, optional) Get deltas for a particular asset instead of DOGPUS.\n"
            "}\n"
            "\nResult:\n"
            "[\n"
            "  {\n"
            "    \"assetName\"  (string) The asset associated with the deltas (DOGPUS for Dogegpu)\n"
            "    \"satoshis\"  (number) The difference of satoshis\n"
            "    \"txid\"  (string) The related txid\n"
            "    \"index\"  (number) The related input or output index\n"
            "    \"height\"  (number) The block height\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"assetName\"  (string) The asset associated with the balance (DOGPUS for Dogegpu)\n"
            "    \"balance\"  (string) The current balance in satoshis\n"
            "    \"received\"  (string) The total number of satoshis received (including change)\n"
            "  },...\n"
            "\n]"
    ...
    if (includeAssets) {
        ...
        for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
            if (!GetAddressIndex((*it).first, (*it).second, DOGPUS, addressIndex)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
            }
        }
...
UniValue getaddresstxids(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
        if (includeAssets) {
            ...
            if (start > 0 && end > 0) {
                if (!GetAddressIndex((*it).first, (*it).second, DOGPUS, addressIndex, start, end)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
                }
            } else {
                if (!GetAddressIndex((*it).first, (*it).second, DOGPUS, addressIndex)) {
