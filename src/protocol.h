...
#ifndef DOGPU_PROTOCOL_H
#define DOGPU_PROTOCOL_H
...
/**
 * Dogpu protocol message types. When adding new message types, don't forget
...
enum ServiceFlags : uint64_t {
    ...
    // set by all Dogpu Core nodes, and is unset by SPV clients or other peers that just want
    // network services but don't provide them.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo protocol request.
    // Dogpu Core does not support this but a patch set called Dogpu XT does.
    // See BIP 64 for details on how this is implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // Dogpu Core nodes used to support this by default, without advertising this bit,
    ...
    // dogpu-development mailing list. Remember that service bits are just
