...
The RPM spec file provided here is for Dogpu-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://dogpu.com/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Dogpu git repository](https://github.com/Masterscooper/Dogegpu) but are not part of
...
Currently only `Source10` is used, for the example `dogpu.conf` file.
...
in the dogpu git repository. At present only an SVG version of the Dogpu
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the dogpu
...
needed when 0.12.1 is released, a proper fix is already in the Dogpu git
master branch.
...
idea to build Dogpu Core against a static version of that library compiled
...
The `dogpu.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `dogpu-server` package. At this point, most RPM based
...
The `dogpu-server` package should have SELinux support. How to properly do
...
Dogpu-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/dogpu/). He has been
packaging Dogpu for Fedora at least since 2012.
...
4. I renamed the `dogpu` package that contains the Qt GUI to `dogpu-core` as
that appears to be how the general population refers to it, in contrast to
`dogpu-xt` or `dogpu-classic`. I wanted to make sure the general population
