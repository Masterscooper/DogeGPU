...
class AssetTest(DogpuTestFramework):
    ...
    def big_test(self):
        ...
        n0.issue("DOGPU1", 1000)
        n0.issue("DOGPU2", 1000)
        n0.issue("DOGPU3", 1000)
        n0.generate(1)
        self.sync_all()

        n0.listassets(asset="DOGPU*", verbose=False, count=2, start=-2)
        ...
        dogpu_assets = n0.listassets(asset="DOGPU*", verbose=False, count=2, start=-2)
        assert_equal(len(dogpu_assets), 2)
        assert_equal(dogpu_assets[0], "DOGPU2")
        assert_equal(dogpu_assets[1], "DOGPU3")
        self.sync_all()
