...
def get_tx_issue_hex(node, to_address, asset_name, asset_quantity=1000, verifier_string="true", units=0, reissuable=1, has_ipfs=0, ipfs_hash="", owner_change_address=""):
    ...
    dogpus_unspent = next(u for u in node.listunspent() if u['amount'] > BURN_AMOUNTS['issue_restricted'])
    dogpus_inputs = [{k: dogpus_unspent[k] for k in ['txid', 'vout']}]
    ...
    outputs = {
        ...
        change_address: truncate(float(dogpus_unspent['amount']) - BURN_AMOUNTS['issue_restricted'] - FEE_AMOUNT),
        to_address: {
            'issue_restricted': {
    ...
    tx_issue = node.createrawtransaction(dogpus_inputs + owner_inputs, outputs)
...
def get_tx_reissue_hex(node, to_address, asset_name, asset_quantity, reissuable=1, verifier_string="", ipfs_hash="", owner_change_address=""):
    ...
    dogpus_unspent = next(u for u in node.listunspent() if u['amount'] > BURN_AMOUNTS['reissue_restricted'])
    dogpus_inputs = [{k: dogpus_unspent[k] for k in ['txid', 'vout']}]
    ...
    outputs = {
        ...
        change_address: truncate(float(dogpus_unspent['amount']) - BURN_AMOUNTS['reissue_restricted'] - FEE_AMOUNT),
        to_address: {
            'reissue_restricted': {
                'asset_name': asset_name,
                'asset_quantity': asset_quantity,
...
def get_tx_issue_qualifier_hex(node, to_address, asset_name, asset_quantity=1, has_ipfs=0, ipfs_hash="", root_change_address="", change_qty=1):
    ...
    dogpus_unspent = next(u for u in node.listunspent() if u['amount'] > BURN_AMOUNTS['issue_qualifier'])
    dogpus_inputs = [{k: dogpus_unspent[k] for k in ['txid', 'vout']}]
    ...
    outputs = {
        ...
        change_address: truncate(float(dogpus_unspent['amount']) - burn_amount - FEE_AMOUNT),
        to_address: {
            'issue_qualifier': {
                'asset_name': asset_name,
                'asset_quantity': asset_quantity,
    ...
    tx_issue = node.createrawtransaction(dogpus_inputs + root_inputs, outputs)
    tx_issue_signed = node.signrawtransaction(tx_issue)
    tx_issue_hex = tx_issue_signed['hex']
    return tx_issue_hex
...
def get_tx_transfer_hex(node, to_address, asset_name, asset_quantity):
    ...
    dogpus_unspent = next(u for u in node.listunspent() if u['amount'] > FEE_AMOUNT)
    dogpus_inputs = [{k: dogpus_unspent[k] for k in ['txid', 'vout']}]
    ...
    outputs = {
        change_address: truncate(float(dogpus_unspent['amount']) - FEE_AMOUNT),
        to_address: {
            'transfer': {
    ...
    tx_transfer = node.createrawtransaction(dogpus_inputs + asset_inputs, outputs)
...
def get_tx_tag_address_hex(node, op, qualifier_name, tag_addresses, qualifier_change_address, change_qty=1):
    ...
    dogpus_unspent = next(u for u in node.listunspent() if u['amount'] > burn_amount)
    dogpus_inputs = [{k: dogpus_unspent[k] for k in ['txid', 'vout']}]
    ...
    tx_tag = node.createrawtransaction(dogpus_inputs + qualifier_inputs, outputs)
...
def get_tx_freeze_address_hex(node, op, asset_name, freeze_addresses, owner_change_address):
    ...
    tx_freeze = node.createrawtransaction(dogpus_inputs + owner_inputs, outputs)
...
class RawRestrictedAssetsTest(DogpuTestFramework):
