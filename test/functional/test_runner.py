...
`test/functional/test_framework/test_framework.py:DogpuTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/dogpu_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and dogpud
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_dogpud = config["components"].getboolean("ENABLE_DOGPUD")
    if not (enable_wallet and enable_cli and enable_dogpud):
        print("No functional tests to run. Wallet, utils, and dogpud must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if dogpud is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "dogpud"]) is not None:
            print("%sWARNING!%s There is already a dogpud process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "DOGPUD" not in os.environ:
        os.environ["DOGPUD"] = build_dir + '/src/dogpud' + exeext
        os.environ["DOGPUCLI"] = build_dir + '/src/dogpu-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `dogpu-cli help` (`rpc_interface.txt`).
