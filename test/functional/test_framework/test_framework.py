...
class DogpuTestFramework:
    """Base class for a dogpu test script.

    Individual dogpu test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true", help="Leave dogpuds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true", help="Don't stop dogpuds after the test execution")
        ...
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"), help="Source directory containing dogpud/dogpu-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: dogpud's were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a dogpud test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple dogpud test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2 ** 16) as log_stderr:
            try:
                ...
                assert 'dogpud exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "dogpud should have exited with an error"
                else:
                    assert_msg = "dogpud should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as dogpud's debug.log with microprecision (so log files can be concatenated and sorted)
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("DogpuRPC")
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run dogpuds:
            for i in range(MAX_NODES):
                datadir = initialize_data_dir(self.options.cachedir, i)
                args = [os.getenv("DOGPUD", "dogpud"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                if i > 0:
        ...
        for i in range(self.num_nodes):
            ...
            initialize_data_dir(self.options.tmpdir, i)  # Overwrite port/rpcport in dogpu.conf
