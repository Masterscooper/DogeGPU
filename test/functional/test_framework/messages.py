...
"""
Dogpu test framework primitive and message structures.
...
COIN = 100000000  # 1 dogpus in Helpers
...
class CAddress:
    """
    Objects that map to dogpud objects, which can be serialized/deserialized
    """
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in dogpud
            if flags != 0:
                self.vin = deser_vector(f, CTxIn)
...
class MsgHeaders:
    ...
    def deserialize(self, f):
        # comment in dogpud indicates these should be deserialized as blocks
